...
8. `references/AI-Impact-on-Field-Service-Benchmarking.docx` (text extract: `references/ai-fs-benchmarking-extract.md`) | **the benchmark evidence base.**  Pierre's industry research: Aquant 2026 benchmarks (first-time fix 77% industry / 88% top / 60% bottom; median resolution 4.5 days; failed visits eating 25% of service cost at the median; 20% of truck rolls unnecessary), Geotab 2026 adoption data (98% taking AI action, but 75% only somewhat confident in readiness), IBM's warning that only 33% of AI initiatives meet ROI targets and 72% fail to scale, TSIA's caution that utilization becomes a misleading metric in the AI era, and McKinsey's predictive-maintenance failure modes.  Rules: cite the source by name in the prose ("Aquant's 2026 benchmark reports...").  Keep the doc's own caveat as content | every one of these vendors sells into this category, so the numbers are directional until the reader runs their own baseline.  The doc's closing insight is a chapter thesis: the fastest path to value is a benchmark-led improvement loop (cut failed visits, improve first dispatch accuracy, increase remote resolution, capture technician knowledge), not a broad AI transformation program.  The eight-KPI scorecard (FTFR, resolution time, failed-visit cost share, remote resolution, wrench time, travel time, SLA attainment, CSAT/NPS) feeds the buyer's checklist chapter.  The data-fragmentation numbers (68% face data-quality challenges, 85% of operators struggle with fragmented data) feed the data-readiness chapter.
9. `Z:\Projects\_WIKI\projects\ai-in-d365-field-service.md` | **the knowledge backbone.**  Pierre's wiki page on AI in D365 Field Service: six AI capability layers, the AI governance framework (his core IP), the RSO decision framework, ROI anchors, and named proof points.  Treat this as the authoritative technical skeleton; the interview adds the stories and voice on top of it.
